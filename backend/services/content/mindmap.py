...
GENERIC_WORDS = {
    ...
    "нужен", "нужно", "нужна", "нужны", "может", "могут", "будет",
    "должен", "должна", "должно", "должны", "следует", "является",
    "являются", "использовать", "использует", "например", "именно",
}
...
def _collect_context(term: str, sentences: List[str], max_sents: int = 5) -> str:
    found = [s for s in sentences if _sentence_contains(term, s)]
    return " ".join(found[:max_sents])


def _build_node_summaries_llm(
    terms: List[str],
    sentences: List[str],
) -> Dict[str, str]:
    contexts = {}
    for t in terms:
        ctx = _collect_context(t, sentences)
        if ctx:
            contexts[t] = _shorten(ctx, 400)

    if not contexts:
        return {t: f"Ключевой термин: {t}" for t in terms}

    term_list = "\n".join(
        f"- {t}: {contexts.get(t, 'нет контекста')}" for t in terms
    )
    prompt = (
        "Ты получаешь список терминов и контекст из документа. "
        "Для каждого термина напиши краткое описание (1-2 предложения), "
        "объясняющее что это и почему важно в контексте документа.\n"
        "Формат ответа — JSON объект {\"термин\": \"описание\"}.\n"
        "Не добавляй ничего кроме JSON.\n\n"
        f"Термины и контекст:\n{term_list}"
    )

    try:
        raw = generate_text(prompt, temperature=0.2, max_tokens=2048)
        raw = raw.strip()
        if raw.startswith("```"):
        ...
        if isinstance(result, dict):
            return {
                t: str(result.get(t, f"Ключевой термин: {t}"))
                for t in terms
            }
    ...
    return {
        t: _shorten(_collect_context(t, sentences) or f"Ключевой термин: {t}")
        for t in terms
    }
...
def _build_graph_summary(
    ...
    strongest = sorted(
        edges.items(), key=lambda x: (-x[1], x[0][0], x[0][1])
    )[:3]
    if not strongest:
        return (
...
def build_graph_data(  # pylint: disable=too-many-locals
    ...
    *,
    ...
    relevance = {
        t: 0.62 * freq_n.get(t, 0) + 0.38 * deg_n.get(t, 0) for t in terms
    }
    ...
    summaries = _build_node_summaries_llm(top, sents)
    ...
    nodes = [
        {
            ...
            "summary": summaries.get(t, f"Ключевой термин: {t}"),
        }
        for t in top
    ]
    edge_list = [
        ...
        for (s, d), w in final_edges.items()
        if w >= min_edge
    ]
...
def render_html(
    ...
    *,
    ...
    graph = build_graph_data(
        text, top_n=top_n, min_freq=min_freq, min_edge=min_edge
    )
  ...
  <div class="page">
    ...
    <aside class="panel">
      ...
      <div class="node-card">
        ...
        <p id="nodeStats" class="stats">\
Кликните на вершину графа, чтобы увидеть краткое описание.</p>
        <p id="nodeSummary" class="muted">\
Здесь появится объяснение выбранного элемента.</p>
      </div>
    </aside>
  </div>
  <script>
    ...
    graphSummaryEl.textContent =
      data?.meta?.summary || "Краткое описание графа недоступно.";
    ...
    function setNodeDetails(node) {{
      if (!node) {{
        ...
        nodeStatsEl.textContent =
          "Кликните на вершину графа, чтобы увидеть краткое описание.";
        nodeSummaryEl.textContent =
          "Здесь появится объяснение выбранного элемента.";
        return;
      }}
      const connected = network.getConnectedNodes(node.id).length;
      nodeTitleEl.textContent = node.label || node.id;
      nodeStatsEl.textContent =
        `Упоминаний: ${{node.value || 0}} | `
        + `Связей: ${{connected}} | `
        + `Релевантность: ${{node.relevance ?? "n/a"}}`;
      nodeSummaryEl.textContent =
        node.summary || "Краткое описание не найдено.";
    }}
    ...
    network.on("click", (params) => {{
      ...
      if (params.edges && params.edges.length > 0) {{
        ...
        nodeSummaryEl.textContent =
          "Эти понятия часто встречаются вместе в одном контексте.";
        return;
      }}
      setNodeDetails(null);
    }});
