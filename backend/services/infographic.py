"""Сервис подготовки JSON-инфографики из RAG-контекста."""
...
def _resolve_models(requested_model: Optional[str]) -> List[str]:
    ...
    if cfg.auto_discover_models:
        try:
            discovered = _discover_models()
            resolved = _dedupe_keep_order(resolved + discovered)
...
def _clean_ocr_noise(text: str) -> str:
    clean = str(text or "")
    clean = re.sub(r"[^\S\r\n]+", " ", clean)
    # Удаляем строки, похожие на оглавление/мусорные коды.
...
def _is_clean_token(token: str) -> bool:
    ...
    # Сильно шумные смешанные токены обычно артефакты OCR.
    if has_lat and has_cyr:
        return False
    # Артефакты вида "ььэ", "фтэ", "шюswэ".
    if re.search(r"[ьъэ]{2,}", token):
        return False
    if re.search(r"[a-z]{1,2}[ьъэ]|[ьъэ][a-z]{1,2}", token, flags=re.IGNORECASE):
        return False
    # Редкие 3-буквенные обрывки без гласных.
...
def _build_facts(results: List[Dict[str, Any]], max_points: int) -> Dict[str, Any]:
    ...
    avg_chunk_len = int(mean(lengths)) if lengths else 0
...
def _call_model_for_plan(model: str, prompt: str) -> Dict[str, Any]:
    ...
    def _parse_with_repair(raw_text: str) -> Dict[str, Any]:
        try:
            ...
            repaired = client.responses.create(
                ...
                input=[
                    {
                        "role": "system",
                        "content": "Исправь ответ до строго валидного JSON. Верни только JSON.",
                    },
                    {"role": "user", "content": raw_text},
                ],
                temperature=0,
            )
            return _extract_json(getattr(repaired, "output_text", "") or "")

    # Предпочитаем Responses API, fallback на Chat Completions.
    try:
...
def generate_infographic_payload(  # pylint: disable=too-many-locals
    query: str,
    ...
    *,
    ...
    """Генерирует JSON-представление для фронтенд-инфографики."""
    if not results:
